 ...
 * @author Inc Tec
 */
public final class TelaTrabalho extends javax.swing.JInternalFrame {
    ...
    private void cadastrar() {
        String sql = "insert into trabalho(nota, obs, disciplina_iddisciplina, aluno_idaluno) values(?,?,?,?)";
        try {
            pst = conexao.prepareStatement(sql);
    ...
    // Metodo Editar
    private void editar() {
        String sql = "update trabalho set nota=?, obs=?, disciplina_iddisciplina=?, aluno_idaluno=? where idtrabalho=?";
        try {
            pst = conexao.prepareStatement(sql);
            pst.setString(1, txtNota.getText());
            pst.setString(2, textSobre.getText());
        ...
        if (confirma == JOptionPane.YES_OPTION) {
            ...
            try {
                ...
                if (txtIdProva.getText().isEmpty()) {
                    ...
                    int adicionado = pst.executeUpdate();
                    if (adicionado > 0) {
                        JOptionPane.showMessageDialog(null, "Trabalho Eliminado com sucesso!");
                        txtNota.setText(null);
                        textSobre.setText(null);
                    }
                }
